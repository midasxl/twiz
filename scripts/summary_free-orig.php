...
$c1=$a*((100-($quarter/$fs))/100);
$c2=$a*((100-($half/$fs))/100);
$c3=$a*((100-($stretch/$fs))/100);
$c4=$a*((100-($finish1/$fs))/100);
if($finish<=3){
$n1=$n1+$c1;
$n2=$n2+$c2;
$n3=$n3+$c3;
...
// rsort : sorts an array in reverse order (highest to lowest).
 ...
 //echo 'Highest is -'.$numbers[0].', Second highest is -'.$numbers[1];
            ...
            if maxRaces exists then the user must have submitted filter post variables
...
$somefig1=$somefig1-$dollarvalue+$bonus;
        ...
        echo '<td>'.number_format($somefig1, 1, '.', '').'</td><td>'.$speedfigure_flag.'</td><td>'.round($combinedc).'</td><td>'.$rsty.'</td></tr>';
